...
class HyperNicheEvaluator:
    ...
    def calculate_consensus_compactness(self, core_runs, consensus_niches, threshold: float, n_permutations: int = 1000):
        ...
        for anchor, members in consensus_niches.items():
            ...
            try:
                hull_area = ConvexHull(member_coords).volume if hard_size >= 3 else 0.0
            except QhullError:
                hull_area = 0.0
            ...
            if len(local_cand_idx) >= hard_size:
                for _ in range(n_permutations):
                    ...
                    try:
                        null_hull_areas.append(ConvexHull(null_coords).volume if hard_size >= 3 else 0.0)
                    except QhullError:
    ...
    def run_full_pipeline(self):
        ...
        for core_runs in self.dataset:
            ...
            # FIX: Point 1 (v7) - Immediately save core-level summary
            num_seeds = len(core_runs)
            majority_thresh = (num_seeds // 2) + 1
        ...
        # =====================================================================
        # Export separate core-level consensus summary (v7 Point 1)
        # =====================================================================
        ...
        # =====================================================================
        # Build Base Patient Aggregation directly from core_consensus_df
        # FIX: Point 1 (v7) - Proper fraction calculation
        # =====================================================================
        ...
        # =====================================================================
        # FIX: Point 2 (v8) - Use np.nan instead of 0.0 for undefined reproducibility
        # =====================================================================
        ...
        # Initialize to 0 so the column exists even if there are no significant results
        ...
        if not consensus_enrichment.empty:
            ...
            if not sig_tests.empty:
                # =====================================================================
                # FIX: Point 2 (v7) - Count unique (core_id, anchor_idx) combinations
                # =====================================================================
                ...
                # Drop initialized column before merging to prevent "_x", "_y" suffix issues
                ...
                # Strongest Biological Signal Per Patient
                ...
                # Export Strongest Significant Enrichment Per Niche
            ...
            # =====================================================================
            # FIX: Point 2 (v7) - Fraction of significant niches safely calculated
            # =====================================================================
        ...
        # =====================================================================
        # USER ROBUSTNESS IMPROVEMENT: Export Patient Summary ALWAYS
        # =====================================================================
