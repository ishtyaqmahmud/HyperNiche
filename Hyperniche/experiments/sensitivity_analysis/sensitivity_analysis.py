...
# def normalize_incidence(
#     H: torch.Tensor,
#     W_e: Optional[torch.Tensor] = None,
#     eps: float = 1e-8
# ) -> Tuple[torch.Tensor, torch.Tensor]:
#     """
#     Weighted hypergraph normalization.

#     Node degree uses weighted incidence:
#         d(v_i) = sum_e H[i, e] * W_e[e]

#     Hyperedge degree uses membership mass:
#         delta(e) = sum_i H[i, e]
#     """
...
#     else:
#         dv = (H * W_e.unsqueeze(0)).sum(dim=1).clamp_min(eps)
...
class Config:
    ...
    # ==========================================================
    min_hyperedge_size: float = 4.0     #3.0 #2.0
    ...
    lambda_degeneracy: float = 0.025     #0.5 #5e-3
...
def main():
    ...
    base_output_dir = os.getenv(
        "HN_BASE_OUTPUT_DIR",
        "/project/sensitivity_analysis_output",
    )
    run_name = os.getenv("HN_RUN_NAME", "default")
    output_dir = os.path.join(base_output_dir, run_name)
    os.makedirs(output_dir, exist_ok=True)

    cfg = Config(in_dim=0, num_classes=0)
    cfg.spatial_k = _env_int("HN_SPATIAL_K", cfg.spatial_k)
    cfg.min_hyperedge_size = _env_float(
        "HN_MIN_HYPEREDGE_SIZE", cfg.min_hyperedge_size
    )
    cfg.lambda_degeneracy = _env_float(
        "HN_LAMBDA_DEGENERACY", cfg.lambda_degeneracy
    )

    if cfg.spatial_k < 1:
        raise ValueError("HN_SPATIAL_K must be at least 1.")
    if not 1.0 <= cfg.min_hyperedge_size <= cfg.spatial_k + 1:
        raise ValueError(
            "HN_MIN_HYPEREDGE_SIZE must be between 1 and spatial_k + 1 "
            f"(received {cfg.min_hyperedge_size} for spatial_k={cfg.spatial_k})."
        )

    print(
        "Run configuration: "
        f"run_name={run_name}, spatial_k={cfg.spatial_k}, "
        f"min_hyperedge_size={cfg.min_hyperedge_size}, "
        f"max_hyperedge_size={cfg.spatial_k + 1}, "
        f"lambda_degeneracy={cfg.lambda_degeneracy}, "
